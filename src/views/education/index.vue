...
const EducationItem = defineAsyncComponent( () => import('./EducationItem.vue') );
...
async function getItems() {
  ...
  OPEN_LOADING_MODAL()
  const [error, response] = await getMedias(searchInput.value, Number(offset), 2, String(lang))
  CLOSE_LOADING_MODAL()
    ...
    <div class="main bg-gray-primary px-8 md:px-10 lg:px-12 right-0 fixed top-0 bottom-0 overflow-y-scroll">
      <div class="flex justify-between items-center mb-10">
        ...
        <div class="flex items-center">
          <div class="bg-white-primary flex px-3 md:px-4 lg:px-6 py-2 md:py-2.5 lg:py-3 rounded w-52 mx-2 lg:mx-8" role="button">
          ...
          <router-link :to="{path: '/education'}" class="bg-orange-primary shadow-lg shadow-black-primary-500/50 flex items-center text-white-primary px-3 md:px-4 lg:px-6 py-2 md:py-2.5 lg:py-3 rounded" role="button">
            <i class="ri-add-line"></i>
            <p class="text-sm px-2 whitespace-nowrap flex items-center gap-1.5"><span class="hidden lg:block">Yangi ta'lim blog</span> qo‘shish</p>
          </router-link>
        </div>
      </div>
