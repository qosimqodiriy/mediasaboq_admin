...
const data = reactive<{offset: number, searchOffset:number, loading: boolean, count:number, list: any[], visible: boolean, value?: {title?: string}, query:  string}>({
  offset: 0,
  searchOffset: 0,
  loading: false,
  count: 0,
  ...
  <ul class="menu">
    <li class="dropdown w-100 dropdown-6 text-black-primary">
      <div  class="flex justify-between items-center w-100 w-full" role="button" @click="data.visible = !data.visible">
        <span>{{data.value == null ? props.title : data.value.title}}</span>
